...
contract DuckBurgerHQ is BuildingKind {
    ...
    function assignUnitToTeam(
        ...
        if (
            ...
            keccak256(abi.encodePacked("duck"))
        ) {
            processTeam(
                dispatcher,
                buildingId,
            ...
            keccak256(abi.encodePacked(team)) ==
            keccak256(abi.encodePacked("burger"))
        ) {
            processTeam(
                dispatcher,
                buildingId,
    ...
    function _start(
        ...
        State state,
        bytes24 buildingId,
        bytes24 duckBuildingID,
        bytes24 burgerBuildingID
    ) private {
        Dispatcher dispatcher = ds.getDispatcher();

        // check teams have at least one each
        uint256 teamDuckLength = uint256(
            state.getData(buildingId, "teamDuckLength")
        );
        uint256 teamBurgerLength = uint256(
            state.getData(buildingId, "teamBurgerLength")
        );
        ...
        dispatcher.dispatch(
            abi.encodeCall(
                Actions.SET_DATA_ON_BUILDING,
                (buildingId, "buildingKindIdDuck", bytes32(duckBuildingID))
            )
        );
        dispatcher.dispatch(
            abi.encodeCall(
                Actions.SET_DATA_ON_BUILDING,
                (buildingId, "buildingKindIdBurger", bytes32(burgerBuildingID))
            )
        );
    ...
    function _claim(
        ...
        require(
            isDuckTeamMember || isBurgerTeamMember,
        ...
        bool isDraw;
        {
            (uint24 duckBuildings, uint24 burgerBuildings) = getBuildingCounts(
                state,
                buildingId
            );
    ...
    function _reset(Game ds, bytes24 buildingId) private {
        ...
        dispatcher.dispatch(
            abi.encodeCall(
                Actions.SET_DATA_ON_BUILDING,
                (buildingId, "teamBurgerLength", bytes32(0))
            )
        );
        dispatcher.dispatch(
            abi.encodeCall(
                Actions.SET_DATA_ON_BUILDING,
                (buildingId, "teamDuckLength", bytes32(0))
            )
        );
        dispatcher.dispatch(
            abi.encodeCall(
        ...
        dispatcher.dispatch(
            abi.encodeCall(
                Actions.SET_DATA_ON_BUILDING,
                (buildingId, "buildingKindIdDuck", bytes32(0))
            )
        );
        dispatcher.dispatch(
            abi.encodeCall(
                Actions.SET_DATA_ON_BUILDING,
                (buildingId, "buildingKindIdBurger", bytes32(0))
            )
        );
    }
    ...
    function getBuildingCounts(
        State state,
        bytes24 buildingInstance
    ) public view returns (uint24 ducks, uint24 burgers) {
        bytes24 duckBuildingKind = bytes24(
            state.getData(buildingInstance, "buildingKindIdDuck")
        );
        bytes24 burgerBuildingKind = bytes24(
            state.getData(buildingInstance, "buildingKindIdBurger")
        );
        ...
        for (uint256 i = 0; i < arenaTiles.length; i++) {
            ...
            if (state.getBuildingKind(arenaBuildingID) == duckBuildingKind) {
                ...
                state.getBuildingKind(arenaBuildingID) == burgerBuildingKind
            ) {
                uint64 constructionBlockNum = state
                    .getBuildingConstructionBlockNum(arenaBuildingID);
                if (
